...
uint8_t Get_LearnNums_Valid(void)
{
    ...
    {
        if(Global_Device.ID[num] == 0)//如果序号1的ID号为0
        {
            if(num>Global_Device.Num)//全局变量中的NUM小于给的NUM；新序列号
            {
                Global_Device.Num = num;//将NUM给到全局变量结构体中
                Flash_LearnNums_Change(num);//FLASH中存放NUM
            }
            return num;
        }
        num++;
...
{
    uint8_t num;
    num = Get_LearnNums_Valid();//得到最新的设备数量，起始值为1
    if(num == 0)
    {
        return RT_ERROR;
    }
    Global_Device.ID[num] = Device_ID;//将ID号放入结构体中
    Global_Device.Bind_ID[num] = 0;//绑定的ID没有，主控没有绑定的ID
    Global_Device.Heart[num] = 1;//心跳计次为1，说明有过
    Flash_ID_Change(num,Device_ID);//FLASH中存放 第几个序号对应第几个ID号
    Flash_Bind_Change(Device_ID,0);//FLASH中存放 这个设备绑定的ID 为0，无；
    Flash_Heart_Change(Device_ID,1);//FLASH中存放 这个设备心跳标志 已经有心跳
    Add_MainNums(); //增加的主控数量存放到FLASH中
    return RT_EOK;
}
...
uint8_t Flash_Set_Heart(uint32_t Device_ID,uint8_t heart)//数据载入到内存中
{
    ...
    while(num)
    {
        if(Global_Device.ID[num]==Device_ID)
        {
            LOG_I("Global_Device.ID[%d]==Device_ID \r\n",num);
            if(heart)
            {
                Global_Device.HeartRecv[num] = 1;
                if(Global_Device.Heart[num]==0)//如果心跳为0，没有心跳
                {
...
uint8_t Flash_Set_UploadFlag(uint32_t Device_ID,uint8_t Flag)//数据载入到内存中
{
    ...
    while(num)
    {
        if(Global_Device.ID[num]==Device_ID)
        {
            Global_Device.UploadFlag[num] = Flag;//设备更新标志为0
            Flash_UploadFlag_Change(Device_ID,Flag);//FLASH中存放更新标志位
            return RT_EOK;
        }
        num--;
    }
