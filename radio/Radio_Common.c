...
void ReceiveData(struct ax5043 *dev)
{
    ...
    LOG_D("Execute first RX_ubDataLen is %d",ubDataLen);//Execute
    while (SpiReadSingleAddressRegister(dev,REG_AX5043_IRQREQUEST0) & 0x01)
    {
        ...
        LOG_I("Execute While RX_ubDataLen is %d",ubDataLen);//Execute
        if (ubDataLen == 0x07)
        {
            ubDataLen = SpiReadSingleAddressRegister(dev,REG_AX5043_FIFODATA);
            LOG_D("Execute third RX_ubDataLen is %d",ubDataLen);//Execute
        }
        ubTemCom &=0x1F;
        LOG_D("Execute RX_ubTemCom is %3X",ubTemCom);//Execute
        switch (ubTemCom)
        {
            case AX5043_FIFOCMD_DATA:
                LOG_D("Execute AX5043_FIFOCMD_DATA is %2X",AX5043_FIFOCMD_DATA);//Execute
                if (!ubDataLen)
                    break;
            ...
            case AX5043_FIFOCMD_RFFREQOFFS:
                LOG_D("Execute AX5043_FIFOCMD_RFFREQOFFS is %2X",AX5043_FIFOCMD_RFFREQOFFS);//Execute
                if (ubDataLen != 3)
                    goto dropchunk;
            ...
            case AX5043_FIFOCMD_RSSI:
                LOG_D("Execute AX5043_FIFOCMD_RSSI is %2X",AX5043_FIFOCMD_RSSI);//Execute
                if (ubDataLen != 1)
                    goto dropchunk;
                {
            ...
            case AX5043_FIFOCMD_TIMER:
                LOG_D("Execute AX5043_FIFOCMD_TIMER is %2X",AX5043_FIFOCMD_TIMER);//Execute
                if (ubDataLen != 3)
                    goto dropchunk;
                SpiReadSingleAddressRegister(dev,REG_AX5043_FIFODATA);
                SpiReadSingleAddressRegister(dev,REG_AX5043_FIFODATA);
            ...
            case AX5043_FIFOCMD_ANTRSSI:
                LOG_D("Execute AX5043_FIFOCMD_ANTRSSI is %2X",AX5043_FIFOCMD_ANTRSSI);//Execute
                if (!ubDataLen)
                    break;
                {
                    SpiReadSingleAddressRegister(dev,REG_AX5043_FIFODATA);
            ...
            default:
            dropchunk:
                LOG_D("Execute Goto dropchunk");//Execute
                if (!ubDataLen)
                    break;
                ubTepm = ubDataLen;
                do
                {
                    LOG_D("Execute ubTepm num is %2X",ubTepm);//Execute
                    SpiReadSingleAddressRegister(dev,REG_AX5043_FIFODATA);
                }
                while (--ubTepm);
                break;
...
void TransmitData(struct ax5043 *dev)
{
    ...
    uint8_t ubFreeCnt,Entercnt = 0,Exitcnt = 0,i=0,j=0;
    Entercnt++;
    LOG_D("Execute Enter %d",Entercnt++);//Execute
    while(1)
    {
        LOG_I("Execute Enter while %d",j++);//Execute
        ...
        switch (dev->ubRFState)
        {
            case trxstate_tx_longpreamble: //A
                LOG_D("Execute TX_longpreamble");//Execute
                if (dev->axradio_txbuffer_cnt==0)
                {
                    dev->ubRFState = trxstate_tx_shortpreamble;
                    LOG_D("Execute Turn to TX_Shortpreamble");//Execute
                    dev->axradio_txbuffer_cnt = dev->config->axradio_phy_preamble_len;
                    LOG_D("Execute txbuffer_cnt is %d",dev->axradio_txbuffer_cnt);//Execute
                    goto shortpreamble;
                }
                if (ubFreeCnt < 4)
                {
                    LOG_D("if ubFreeCnt < 4 go to fifocommit");
                    goto fifocommit;
                }
                ubFreeCnt = 7;
                if (dev->axradio_txbuffer_cnt < 7)
                {
                    LOG_D("if dev->axradio_txbuffer_cnt < 7 ");
...
//                Exitcnt++;
                LOG_D("Execute TX_shortpreamble %d",i);//Execute
            shortpreamble:
                LOG_D("Execute goto shortpreamble Handle");//Execute

                LOG_D("Execute axradio_txbuffer_cnt is %d",dev->axradio_txbuffer_cnt);//Execute
                if (dev->axradio_txbuffer_cnt == 0)
                {
                    LOG_D("Execute if axradio_txbuffer_cnt == 0");//Second Execute
                    if (ubFreeCnt < 15)
                    {
                        LOG_D("if ubFreeCnt < 15 to fifocommit");
                        goto fifocommit;
                    }
                    if (dev->config->axradio_phy_preamble_appendbits) {
                        LOG_D("if phy_preamble_appendbits");
                        uint8_t byte;
                        ...
                        if (SpiReadLongAddressRegister(dev,REG_AX5043_PKTADDRCFG) & 0x80) {
                            LOG_D("if PKTADDRCFG ture so msb first");
                            ...
                            LOG_D("if PKTADDRCFG false so  lsb first");
                    ...
                    {
                        LOG_D("Execute axradio_framing_synclen is %d ",dev->config->axradio_framing_synclen);//Execute
                        ...
                        LOG_D("Execute ubRFState ===> trxstate_tx_packet");//Execute
                        LOG_D("Execute shortpreamble break %d");//Execute
                        break;
                    }
                }
                LOG_D("Execute ubFreeCnt is %d",ubFreeCnt);//Execute
                if (ubFreeCnt < 4)
                {
                    LOG_D("if (ubFreeCnt < 4) goto fifocommit");
                    goto fifocommit;
                }
                ubFreeCnt = 255;
                if (dev->axradio_txbuffer_cnt < (255*8))
                {
                    LOG_D("if (axradio_txbuffer_cntt is %d < (255*8)",dev->axradio_txbuffer_cnt);
                    ubFreeCnt = dev->axradio_txbuffer_cnt >> 3;
                }

                if (ubFreeCnt)
                {
                    LOG_D("Execute if ubFreeCnt is %d",ubFreeCnt);//Execute
                    dev->axradio_txbuffer_cnt -= ((uint16_t)ubFreeCnt) << 3;
                    LOG_D("<< 3 axradio_txbuffer_cntt is %d",dev->axradio_txbuffer_cnt);
                ...
                }
                {
                    ...
                    LOG_D("Execute axradio_txbuffer_cnt is 0");//Execute
                    SpiWriteSingleAddressRegister(dev,REG_AX5043_FIFODATA ,AX5043_FIFOCMD_DATA | (2 << 5));
                    SpiWriteSingleAddressRegister(dev,REG_AX5043_FIFODATA, 0x1C);
                    if (SpiReadLongAddressRegister(dev,REG_AX5043_PKTADDRCFG) & 0x80)
                    {
                        // msb first -> stop bit below
                        LOG_D("msb first -> stop bit below");
                    ...
                    {
                        // lsb first -> stop bit above
                        LOG_D("lsb first -> stop bit above");
                        byte &= 0xFF >> (8-ubFreeCnt);
                        byte |= 0x01 << ubFreeCnt;
                    }
                    SpiWriteSingleAddressRegister(dev,REG_AX5043_FIFODATA, byte);
            ...
            case trxstate_tx_packet:  //C
                LOG_D("Execute TX_packet");//Execute
                LOG_D("Execute TxLen is %d",dev->TxLen);//Execute
                if (dev->TxLen < 11)goto fifocommit;//No Execute
    ...
    pktend:
        LOG_D("Execute TX_pktend");//Execute
        SpiWriteSingleAddressRegister(dev,REG_AX5043_FIFOSTAT, 4); // commit
        ...
        LOG_D("Execute Exitcnt %d",Exitcnt++);//Execute
}
