...
{
    Flash_Set_Rssi(device_id,rssi);//将收到的设备RSSI 写入 全局变量中
    char *Buf = rt_malloc(20);
    sprintf(Buf,"%ld",device_id);
    mcu_dp_enum_update(101,rssi,Buf,my_strlen(Buf)); //终端或者门控 数据上报;
...
void Device_Add2Flash_Wifi(uint32_t device_id,uint32_t from_id)
{
    ...
    {
        LOG_I("device_id>=10000000 && device_id<20000000 \r\n");
        if(Flash_Get_Key_Valid(device_id) == RT_ERROR)//如果此ID号为主控ID，且在 全局变量 查询是否为首次添加
        {
            MainAdd_Flash(device_id);
        }
        if(Remote_Get_Key_Valid(device_id) == RT_ERROR)//如果此ID号为主控ID，且在 WIFI远程控制数据结构体 查询是否为首次添加
        {
            Flash_Set_UploadFlag(device_id,0);//设备更新WIFI的状态标志为0，代表没有上传更新数据
            Main_Add_WiFi(device_id);//主控加入到WIFI模块当中
        }
    }
    if(device_id>=20000000 && device_id<30000000)
    {
        LOG_I("device_id>=20000000 && device_id<30000000 \r\n");
    ...
    {
        LOG_I("device_id>=30000000 && device_id<40000000 \r\n");
        if(Flash_Get_Key_Valid(device_id) == RT_ERROR)
        {
...
void Upload_Door_ID(uint32_t device_id,uint32_t from_id)
{
    ...
    sprintf(Doorbuf,"%ld",device_id);
    mcu_dp_value_update(107,from_id,Doorbuf,my_strlen(Doorbuf)); //绑定的主控ID上报;
    mcu_dp_value_update(108,device_id,Mainbuf,my_strlen(Mainbuf)); //门控ID上报;
...
{
    if(Flash_Get_UploadFlag(device_id)==0)//如果WIFI询问的ID，之前都没有更新过，是首次更新数据
    {
        Flash_Set_UploadFlag(device_id,1);//置更新标志位
        if(device_id>=10000000 && device_id<20000000)//如果是主控
        {
            Upload_Main_ID(device_id); //上传主控ID ，及其执行器的状态，给WIFI模块
            Upload_Door_ID(GetDoorID(device_id),device_id);//上传主控其所属的门控ID，状态，RSSI给WIFI模块
            Reset_Main_Warn(device_id);//复位主控ID 的所以 报警DP点
        }
        else if(device_id>=20000000 && device_id<30000000) //如果是终端
        {
            Reset_Slave_Warn(device_id);//复位终端的报警DP
            Upload_Slave_ID(device_id,GetBindID(device_id));//上报终端绑定的主控，及上报终端的RSSI
        }
        else if(device_id>=30000000 && device_id<40000000) //如果是门控
        {
            Upload_Door_ID(device_id,GetBindID(device_id));//上传门控相关的DP点数据
        }
    }
}
